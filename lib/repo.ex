defmodule Repo do
  ...
  def load(:character, id) do
    ...
    data |> Map.merge(%{account_id: account_id, name: name})   #把account_id , name 这两条数据合到data中
    # Logger.debug "Repo -> load(:character, id) end"
  end

  def save(:character, id, character) do
    {%{account_id: account_id, name: name}, data} = Map.split(character, [:account_id, :name]) #data保存除去account_id和name之外的数据
    :dets.insert(@character_table, {id, account_id, name, data})
  end

  #为第一次加载的角色初始化数据
  def migrate(:character, vsn, character) do   #如果character中的 vsn 为0，则创建character的数据 其中vsn为传进来的vsn
  Logger.debug "Repo -> migrate(:character, vsn, character) do"
    case Map.get(character, :vsn, 0) do         #如果character中的 vsn 为传进来的vsn，则不对character数据做修改
      0 ->              #新角色第一次加载数据
      # Logger.debug "case 0"
        character
        |> Map.merge(%{level: 1,
                       stats: %{},
                       points: %{hp: 100, exp: 0},
                       currencies: %{coin: 10000, gold: 0, bindGold: 1000000},
                       bank: %{coin: 0 } , 
                       pos: %{map: 1, x: 10, y: 10},
                       bag: %{0 => %{id: 3, count: 1000}, 1 => %{id: 1, count: 1000}, 
                       2 => %{id: 2, count: 1000}, 3 => %{id: 4, count: 1000},  
                       4 => %{id: 5, count: 1000}, 5 => %{id: 6, count: 1000},
                       6 => %{count: 1000, id: 1001}, 7 => %{count: 1000, id: 1002}, 
                       8 => %{count: 1000, id: 201}, 9 => %{count: 1000, id: 202}, 
                       10 => %{count: 1000, id: 203}, 11 => %{count: 1000, id: 301},
                       12 => %{count: 1000, id: 401}, 13 => %{count: 1000, id: 501}, 
                       14 => %{count: 1000, id: 802}, 15 => %{count: 1000, id: 810}},
                       bagCells: @init_cells,
      ...
      1 -> character |> Map.put(:c, 0) |> Map.put(:vsn, 1)
      ## 2 -> character |> Map.put(:d, 0) |> Map.put(:vsn, 2)
      ## 3 -> character |> Map.put(:e, 0) |> Map.put(:vsn, 3)
      ^vsn -> character
      _ -> character
    end
