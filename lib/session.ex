defmodule Session do
  ...
  def handle_cast(
        {:reply, reply},
        %{state: :playing, transport: transport, socket: socket} = state
      ) do
  ...
  def handle_cast(_, %{state: :playing, transport: transport, socket: socket} = state) do
    Logger.debug(
      "handle_cast(_, %{state: :playing, transport: transport, socket: socket} = state)"
    )
  ...
  defp handle_request({:account, :auth}, [account], %{state: :wait_auth} = state) do
    ...
    {:reply, {{:account, :auth}, "ok"},
     %{state | state: :authed} |> Map.put_new(:account_id, account_id)}
  end
  ...
  defp handle_request(
         {:character, :create},
         [name, gene],
         %{state: :authed, account_id: account_id} = state
       ) do
