...
defmodule CharacterCompute do
    ...
    def attributecompute(attribute) do
        attack_battlepower = ((attribute[:attack] || 0) + (attribute[:sunderArmor] || 0) + (attribute[:hitTarget] || 0) + (attribute[:criticalHit] || 0)) * 10
        defence_battlepower = (attribute[:life] || 0) * 0.5 +  ((attribute[:defence] || 0) + (attribute[:evasion] || 0) + (attribute[:tenacity] || 0)) * 10
