defmodule Pet do
  ...
  # 升星所需要的item
  @raise_items %{
    1 => %{name: "pet_bless_card", blessing: 150},
    2 => %{name: "pet_pill", blessing: 10}
  }
  ...
  def swallow(equip, {id, %{pet: %{exp: exp} = pet, bag: bag}}) do
    with true <-
           equip
           |> Map.to_list()
           |> Enum.map(fn {id, count} -> Inventory.enough?(bag, id, count) end)
           |> Enum.all?(fn x -> true == x end) do
      cost = equip |> Map.to_list() |> Enum.map(fn {id, count} -> {:item, id, count} end)

      {cost_events, [new_bag, new_exp]} =
        equip
        |> Map.to_list()
      ...
      context = %{
        action: {},
        events: [swallow_events, cost_events],
        changed: %{bag: new_bag, pet: new_pet}
      }
  ...
  def hh(new_hh_id, {id, %{pet: %{pet_id: pet_id, actived: actived, hh: hh_id} = pet}}) do
    if (new_hh_id == pet_id and new_hh_id != hh_id) or
  ...
  def pet_advanced(
        eat_item_id,
        count,
        {id, %{pet: %{pet_id: _pet_id, blessing: blesssing, actived: actived} = pet, bag: bag}}
      ) do
    with true <- Inventory.enough?(bag, eat_item_id, count) do
      ...
      context =
        if Map.has_key?(actived, actived_id) do
          ...
          %{
            action: {},
            events: [pet_events, cost_events, active_events],
            changed: %{bag: new_bag, pet: new_pet}
          }
        end
