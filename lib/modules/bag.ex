defmodule Bag do
  ...
  @spec sell(any, any, any, {any, %{bag: map, currencies: any}}) ::
          :ok | {:resolve, {:sell, {any, any}}, [...]}
  def sell(index, count, currencyType \\ :bindGold, {_id, %{bag: bag, currencies: currencies}} = all) do
    ...
    with {item, stock} when stock >= count <- Inventory.get(bag, index),
      ...
      # event发送消息
      event = {:sell, {:bag, index}}

      # 将消息发给avatar，avatar对玩家数据进行修改
      {:resolve, event, Effect.from_cost_rewards(lost_effects, income_effects)}
    else
      _ -> :ok
    end
  end
  ...
  def buy(index, count, currencyType \\ :bindGold, {_id, %{bag: bag, currencies: currencies}} = all) do
    ...
    with item <- Inventory.getItem(bag, index),
      ...
      {:resolve, {:buy, {:bag, index}}, Effect.from_cost_rewards(cost_effects, get_effects)}
    else
      _ ->
  ...
  def bag2warehouse(index, {_id, %{bag: bag, warehouse: warehouse}} = all) do
    ...
    with {item, stock} <- Inventory.get(bag, index),
      ...
      {:resolve, {:sell, {:bag, index}}, Effect.from_cost_rewards(lost_effects, get_effects)}
    else
      _ ->
