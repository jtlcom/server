defmodule Bag do
    ...
    #展示所有信息
    def show_bag({_id, %{bag: bag, currencies: currencies}} = all) do
        Logger.debug "all: #{inspect all, pretty: true}"
    end
    # 出售 默认出售物品后获得:bindGold
    def sell(index, count,currencyType \\ :bindGold, {_id, %{bag: bag, currencies: currencies}} = all) do
        # Logger.debug("sell ")
        # Logger.debug "all: #{inspect all, pretty: true}"
        ...
        with {item, stock} when stock >= count <- Inventory.get(bag, index), #获取bag中index物品的信息
            income <-  Logical.sell(item.id) |> Map.take(List.wrap(currencyType)) || [] #从item获取物品相关信息（主要是这个物品各种货币应该卖多少钱）
        do
            ...
            income_effects = income |> Enum.map(fn {currency, price} -> {:gain, currency, price*count} end) #计算卖出后的收益
    ...
    def buy(index, count,currencyType \\ :bindGold, {_id, %{bag: bag, currencies: currencies}} = all) do
        ...
        with item <- Inventory.getItem(bag, index), #获取bag中index物品的信息
            ...
            price when totalmoney >= price * count  <-  Map.get( Logical.buy(item.id), currencyType, totalmoney + 1) #获取买这个物品的单价，并判断自己这种货币currencyType \\ :bindGold 够不够
    ...
    #将背包的某个物品全部存入仓库
    def bag2warehouse(index, {_id, %{bag: bag, warehouse: warehouse}} = all) do
        ...
        #将仓库的某个物品全部存入背包
        def warehouse2bag(index, {_id, %{bag: bag, warehouse: warehouse, bagCells: bagCells}} = all) do
    ...
    def use(index, count, {_id, %{bag: bag, gene: gene} = data}) do
        with {%{id: item_id} = item, stock} <- Inventory.get(bag, index),
            true <- Action.match?(Logical.require(item_id), data),   # Action.match?  判断requirements （需要的条件） ，data是否满足，requirement可以是bag的东西，玩家等级（level），vip等级或其他属性
